...
`;
  ...
  head.insertAdjacentHTML("afterbegin", a_style_new);
};
...
`;
    ...
    document
      ...
      .insertAdjacentHTML("beforeend", productElement);
  });

  if (products.length === 0) {
    document.querySelector(".a_new_container").style.display = "none";
  }
  ...
  function showPopup() {
    const anatomi_sepet_popUp = document.querySelector(".a_new_container");
    anatomi_sepet_popUp.classList.add("anatomi_sepet_popUp-active");
    isPopupShown = true;
  }

  function hidePopup() {
    const anatomi_sepet_popUp = document.querySelector(".a_new_container");
    anatomi_sepet_popUp.classList.remove("anatomi_sepet_popUp-active");
    isPopupShown = false;
  }
...
const products = JSON.parse(localStorage.getItem("paid-products"));

if (products) {
  if(window.location.href.includes("sepet") || window.location.href.includes("payment")) {
    throw new Error("anatomi_popUp page error");
  }else {
    init_80();
  }
}else {
