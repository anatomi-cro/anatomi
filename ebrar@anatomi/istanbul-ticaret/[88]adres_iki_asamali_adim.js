  ...
  const organizeDivs = () => {
    ...
    if (hasGirisYapContainer) {
      allDivs.forEach((div, index) => {
        if (index === 3) {
          div.classList.add("anatomi_adsoyad_container");
        }
        if (index < 8) {
      ...
      const adsoyadContainer = firstStepContainer.querySelector(
        ".anatomi_adsoyad_container"
      );

      const originalParent = adsoyadContainer.parentNode;
      const originalNextSibling = adsoyadContainer.nextElementSibling;

      const moveAdSoyad = () => {
        if (select.value === "1") {
          const firstInSecondStep = secondStepContainer.children[1];

          if (firstInSecondStep) {
            secondStepContainer.insertBefore(
              adsoyadContainer,
              firstInSecondStep.nextElementSibling
            );
          } else {
            secondStepContainer.appendChild(adsoyadContainer);
          }

          console.log("ad-soyad second stepte ilk divin sonrasına taşındı");
        } else {
          if (originalNextSibling) {
            originalParent.insertBefore(adsoyadContainer, originalNextSibling);
          } else {
            originalParent.appendChild(adsoyadContainer);
          }
          console.log("ad-soyad tekrar first stepte");
        }
      };
      moveAdSoyad();

      select.addEventListener("change", moveAdSoyad);
    } else {
      allDivs.forEach((div, index) => {
        if (index === 2) {
          div.classList.add("anatomi_adsoyad_container");
    ...
    if (hasGirisYapContainer) {
      ...
      const checkFormValidity = () => {
        ...
        if (select.value === "1") {
          ...
          if (email && !isValidEmail(email.value)) allFilled = false;
          if (firmaAdi && !firmaAdi.value.trim()) allFilled = false;
          if (vergiDairesi && !vergiDairesi.value.trim()) allFilled = false;
          if (vergiNo && !vergiNo.value.trim()) allFilled = false;
        }
      ...
      firstStepDevamEtBtn.addEventListener("click", (e) => {
        ...
        if (
          select.value === "0" &&
          fullName &&
          !isValidFullName(fullName.value)
        ) {
      ...
      const checkFormValidity = () => {
        ...
        const firmaValid = firmaAdi?.value.trim();
        const vergiDairesiValid = vergiDairesi?.value.trim();
        const vergiNoValid = vergiNo?.value.trim();

        if (select.value === "0") {
          allFilled = fullNameValid && isValidFullName(fullName.value);
        }

        if (select.value === "1") {
          allFilled = firmaValid && vergiDairesiValid && vergiNoValid;
        }

        firstStepDevamEtBtn.disabled = !allFilled;
        firstStepDevamEtBtn.classList.toggle("disabled-link", !allFilled);
      };
