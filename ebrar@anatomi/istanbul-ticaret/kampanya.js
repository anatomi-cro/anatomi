const kampanya_info_html = () => {};

const kampanya_info_css = () => {
    const style = `
      <style>    
    ...
    const addInfoSectionDesktop = () => {
        ...
        if(!itemContainer){
            ...
            if (itemContainer) {
                ...
                infoSection.textContent = "Sepetini 5.000 TL’ye tamamla %10 İndirim Kazan!";
...
const kampanya_info_init = () => {
    kampanya_info_html();
    kampanya_info_css();
    kampanya_info_js();
};

const kampanya_info_condition = () => {
    const device = window.innerWidth <= 768;
    return device;
};

if (kampanya_info_condition()) {
    kampanya_info_init();
}
