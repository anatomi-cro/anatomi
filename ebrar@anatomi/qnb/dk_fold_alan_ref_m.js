...
const dk_fold_alan_ref_m_js = () => {};

const dk_fold_alan_ref_m_init = () => {
  dk_fold_alan_ref_m_html();
  dk_fold_alan_ref_m_css();
  dk_fold_alan_ref_m_js();
};

const dk_fold_alan_ref_m_condition = () => {
  const device = window.innerWidth <= 768;
  return device;
};

setInterval(() => {
  if (dk_fold_alan_ref_m_condition()) {
    dk_fold_alan_ref_m_init();
  }
}, 50);
