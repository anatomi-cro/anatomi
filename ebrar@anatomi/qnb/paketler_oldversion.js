 ...
 <div class="anatomi-packages-container">
      <div class="anatomi-packages-content">
        <div class="anatomi-packages-title">
          <h1><span>e-Fatura</span> ve diğer e-Dönüşüm Paketleri</h1>
        </div>
  ...
  const container = document.querySelector(
    "body > main > div.uk-position-relative.uk-margin-32\\@m.notranslate",
  );
...
const paketler_redesign_js = () => {
  ...
  function removeOtherSections() {
    const section1 = document.querySelector("#sections-1");
    if (!section1) return;
    const section2 = document.querySelector("#sections-2");
    if (!section2) return;
    const section3 = document.querySelector("body > main > div:nth-child(7)");
    if (!section3) return;
    const section4 = document.querySelector("body > main > div:nth-child(8)");
    if (!section4) return;
    const nav = document.querySelector(
      "body > main > div.uk-position-z-index.packages-tabs.uk-sticky.notranslate",
    );
    if (!nav) return;
    const idSections3 = document.querySelector("#sections-3");
    if (!idSections3) return;
    const idSections4 = document.querySelector("#sections-4");
    if (!idSections4) return;
    ...
    section4.remove();
    nav.remove();
    if (idSections3.style.display !== "none") {
      idSections3.style.display = "none";
    }
    idSections4.remove();
  }
  ...
  function oldKontorSelect(selectEl) {
    const kontors = document.querySelectorAll("#list-dropdown-2-2 > ul > li a");
    ...
    kontors.forEach((kontor) => {
      ...
      console.log(dataName.replace("Paketi", ""), value);
  ...
  function oldButtonClick(element) {
    const oldButton = document.querySelector(
      ".trialpackage.trialpackage-TK50_12Ay_Sureli",
    );
  ...
  function startClick() {
    ...
    const checkText = document.querySelector(
      "#uk-switcher-16 > div > div:nth-child(2) > div > div.dropdown-stretch.notranslate > a > p > span",
    ).textContent;
    if (!kontors || kontors.length === 0) return;
    kontors.forEach((kontor) => {
      const dataName = kontor.getAttribute("data-name");
  ...
  function fireViewItemList() {
    const el = document.querySelector("#sections-1");
    if (!el) {
      console.warn("#sections-1 bulunamadı");
      return;
    }
...
setTimeout(() => {
  ...
  const testID = "1004165225";
  const state = "varyasyon";
  const url = "paketler";
  const className = "anatomi-packages-container";
  const divID = null;
