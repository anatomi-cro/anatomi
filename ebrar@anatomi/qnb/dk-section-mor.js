...
@media (max-width: 768px) {
  ...
  const dk_section_degisikligi_js = () => {
    ...
    const banner = document.querySelector("body > main > div.uk-position-relative.notranslate > section > div")
    if (banner && ucAdımSection) {
      banner.insertAdjacentElement("beforeend", ucAdımSection);
    }

    const kutuContainer = document.querySelector("body > main > div.uk-position-relative.notranslate > section > div > section > div > div.uk-grid.uk-grid-20.uk-child-width-1-3\\@m.uk-margin-32.uk-margin-medium\\@m.uk-grid-match.notranslate")
    if (kutuContainer) {
    ...
    function handleClick() {
      ...
      if (containers) {
       ...
       const wrapperContainer = document.querySelectorAll("body > main > div.uk-position-relative.notranslate > section > div > section > div > div.uk-grid.uk-grid-20.uk-child-width-1-3\\@m.uk-margin-32.uk-margin-medium\\@m.uk-grid-match.notranslate > div")
      wrapperContainer.forEach((item) => {
        item.classList.add("anatomi-wrapper-container");
      })


        const span = document.querySelectorAll("body > main > div.uk-position-relative.notranslate > section > div > section > div > div.uk-grid.uk-grid-20.uk-child-width-1-3\\@m.uk-margin-32.uk-margin-medium\\@m.uk-grid-match.uk-grid-stack.notranslate > div > div > span")
        span.forEach((item) => {
            item.classList.add("anatomi-span");
        })

      const wrapper = document.querySelectorAll("body > main > div.uk-position-relative.notranslate > section > div > section > div > div.uk-grid.uk-grid-20.uk-child-width-1-3\\@m.uk-margin-32.uk-margin-medium\\@m.uk-grid-match.notranslate.anatomi-box-container > div > div")
