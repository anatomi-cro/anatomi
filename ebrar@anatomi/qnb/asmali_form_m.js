...
const asamali_form_js = () => {
    ...
    const stickyContainer = document.querySelector("body > main > section > div > div.uk-grid.uk-grid-row-collapse.uk-grid-stack.notranslate > div.uk-width-1-3\\@m.uk-grid-margin.uk-first-column.notranslate > div.toggle-button.toggle-buttonn.uk-background-secondary.uk-padding-small-vertical.uk-padding-20-horizontal.uk-position-fixed.uk-position-bottom.uk-light.uk-hidden\\@m.sticky-price-summary.notranslate > div")
    if (stickyContainer && !stickyContainer.classList.contains("anatomi_sticky_price_summary")) {
        stickyContainer.classList.add("anatomi_sticky_price_summary");
    }
    const outerSpan = document.querySelector(".uk-button.uk-button-link.uk-gap-4.uk-text-bold");

    if (outerSpan) {
      const textNode = Array.from(outerSpan.childNodes).find(node => node.nodeType === Node.TEXT_NODE);
      if (textNode) {
        textNode.remove();
      }
    }
