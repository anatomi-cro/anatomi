const sigortaladım_teklif_listeleme_d_html = () => {
  ...
  const sigortaladım_teklif_listeleme_d_js = () => {
   ...
   const generateInfoDetailMobile = () => {
      const infoDetail = document.querySelector(
        "#offerList > div > div.offer-form-info-container > div.list-controls-container > div.info-detail"
      );
      if (infoDetail && !infoDetail.classList.contains("anatomi_info_detail")) {
      ...
      if (aracInfoModel && infoBrand && !infoBrand.contains(aracInfoModel)) {
        infoBrand.appendChild(aracInfoModel);
      }
    ...
    const handleResponsiveInfoDetail = () => {
      if (window.innerWidth < 768) {
        setInterval(generateInfoDetailMobile, 100);
      } else {
       setInterval(generateInfoDetail, 100); 
      }
    }; 
    handleResponsiveInfoDetail();
 
    window.addEventListener("resize", handleResponsiveInfoDetail);
   ...
   let newContainer = document.querySelector(".anatomi_btn_container");
      const packagesContainer = document.querySelector("#offerList > div > div.packages.anatomi_packages")
      if (!newContainer) {
       
        newContainer = document.createElement("div");
        newContainer.classList.add("anatomi_btn_container");

     
        packagesContainer.appendChild(newContainer);
      }

   
      const buttons = document.querySelectorAll(".anatomi_btn_package1, .anatomi_btn_package2, .anatomi_btn_package3, .anatomi_btn_package4");

    
      buttons.forEach(button => {
        if (!newContainer.contains(button)) { 
  ...
  const checkScreenSize = () => {
      const offerItems = document.querySelectorAll("#offerList > div > div.offer-list-container.casco > div > div.offer-item");
  
      offerItems.forEach((offerItem) => {
          const offerButton = offerItem.querySelector("div.price.anatomi_price_container > button");
          const logoButton = offerItem.querySelector("div.logo > button");
  
          if (!offerButton || !logoButton) return;
  
          let responsiveContainer = offerItem.querySelector(".responsive-container");
  
          if (window.innerWidth < 768) {
              if (!responsiveContainer) {
                  responsiveContainer = document.createElement("div");
                  responsiveContainer.classList.add("responsive-container");
                  offerItem.appendChild(responsiveContainer);
              }
  
              if (offerButton.parentNode !== responsiveContainer) {
                  responsiveContainer.appendChild(offerButton);
              }
              if (logoButton.parentNode !== responsiveContainer) {
                  responsiveContainer.appendChild(logoButton);
              }
  ...
  const moveButtonsBack = () => {
    const offerItems = document.querySelectorAll("#offerList > div > div.offer-list-container.casco > div > div.offer-item");
  
    offerItems.forEach((offerItem) => {
      const responsiveContainer = offerItem.querySelector(".responsive-container");
      const priceContainer = offerItem.querySelector(".price.anatomi_price_container");
      const logoContainer = offerItem.querySelector(".logo > div");
  
      const offerButton = offerItem.querySelector("button.anatomi_offer_buy_button");
      const logoButton = offerItem.querySelector("button.anatomi_offer_teminat_detail_button");
  
      if (window.innerWidth >= 768) {
        if (responsiveContainer && offerButton && offerButton.parentNode === responsiveContainer) {
          priceContainer.appendChild(offerButton);
        }
        if (responsiveContainer && logoButton && logoButton.parentNode === responsiveContainer) {
          logoContainer.insertAdjacentElement("afterend", logoButton);
        }
        if (responsiveContainer) {
...
const filtreleme_js = () => {
  ...
  const handle_change_text = () => {
   ...
   console.log(count)
    ...
    const inputs = document.querySelectorAll(".anatomi_filtre_item input");
    ...
    if (
      ...
      text.textContent = `Sana Özel ${count} Kasko Teklifi hazırladık.`;
    } else if (count == 0 && isAnyChecked > 0) {
      console.log("deneme")
      let button_text;
