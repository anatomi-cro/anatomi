...
//variables
const anatomi_body = document.querySelector("body");
...
const mutationObserver = new MutationObserver((entries) => {
  entries.forEach((item, index) => {
    if (item.target.lastChild.className == "pWrapper" && index == 0) {
      ...
      document
        .querySelectorAll("#basketPopupItems div .whiteBg.box-border")[1]
        .remove();
      //butonlar
      ...
      const a_text_change = (document.querySelector(
        ".pButtons"
      ).children[0].innerText = "Alışverişe Devam Et");
      cartContainer.insertAdjacentHTML("beforeend", a_html_a);
      ...
      document
        .querySelector(".anatomi_container .product_buttons")
        .appendChild(buttonBasket);
      document
        .querySelector(".anatomi_container .product_buttons")
        .appendChild(buttonContinue);
...
//when container is visible
mutationObserver.observe(anatomi_body, {
  childList: true,
  attributes: true,
});
...
//style implement
document.querySelector("head").insertAdjacentHTML("beforeend", a_style_a);
}