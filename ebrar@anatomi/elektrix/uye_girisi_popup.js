const uye_girisi_popup_html = () => {
  const html = `
    `;
};
const uye_girisi_popup_css = () => {
  const style = `
      <style id="anatomi_auth_popUp_fix">
  ...
  const elementStyle = document.querySelector("#anatomi_auth_popUp_fix");
  if (!elementStyle) {
    head.insertAdjacentHTML("beforeend", style);
  }
};
const uye_girisi_popup_js = () => {
  const myPopupWin = document.querySelector("#myPopupWin");

  if (myPopupWin) {
    myPopupWin.classList.add("anatomi_auth_popUp");
  }
...
const uye_girisi_popup_init = () => {
  uye_girisi_popup_html();
  uye_girisi_popup_css();
  uye_girisi_popup_js();
};
const uye_girisi_popup_condition = () => {
  return window.innerWidth <= 768;
};
if (uye_girisi_popup_condition()) {
  setInterval(() => {
    const myPopupWin = document.querySelector(".anatomi_auth_popUp");
    if (!myPopupWin) {
      uye_girisi_popup_init();
    }
  }, 100);
}
