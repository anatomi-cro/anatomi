const sompo_urun_detay_trafik_html = () => {
  const html = `
  `;
};
const sompo_urun_detay_trafik_css = () => {
  const style = `
       <style>
...
const sompo_urun_detay_trafik_js = () => {
    ...
    if(getOfferBtn){
      getOfferBtn.textContent="TRAFİK SİGORTASI TEKLİFİ AL"
 ...
 sliderContainer.classList.add("anatomi_slider_container");
  ...
  sliderImg.src =
    "https://anatomi.s3.eu-north-1.amazonaws.com/sompo_trafik_kare_image.jpg";
...
slideTitle.textContent =
  "Trafik teklif hesaplama formunu doldurarak online olarak aracınız için en uygun trafik teklifini alabilirsiniz";
...
if(window.innerWidth < 768){
    ...
    secondSlideMobilTitle.textContent = "Trafik Sigortası";
    secondSlide.appendChild(secondSlideMobilTitle);
}


const items = [
  " Zorunlu Trafik Sigortası'nda <strong>uygun fiyatlar</strong>",
  "<strong> 500 TL Hediye Kartı </strong>Kazanma İmkanı!",
  " <strong>Peşin fiyatına 3 taksite</strong> kadar ödeme imkanı",
  "<strong>%50’ye varan</strong> hasarsızlık indirimi",
];
...
const sompo_urun_detay_trafik_init = () => {
  sompo_urun_detay_trafik_html();
  sompo_urun_detay_trafik_css();
  sompo_urun_detay_trafik_js();
};
const sompo_urun_detay_trafik_condition = () => {   

  const location = window.location.href === "https://www.somposigorta.com.tr/trafik-sigortasi";
    
    const device = window.innerWidth >= 768;

    return device;
};
if (sompo_urun_detay_trafik_condition()) {
  sompo_urun_detay_trafik_init();
}
