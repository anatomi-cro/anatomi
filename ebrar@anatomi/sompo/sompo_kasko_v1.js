const sompo_urun_detay_kasko_html = () => {
  const html = `
  `;
};
const sompo_urun_detay_kasko_css = () => {
  const style = `
    <style>
...
const sompo_urun_detay_kasko_js = () => {
  ...
  if (getOfferBtn) {
    getOfferBtn.textContent = "KASKO TEKLİFİ AL";
  ...
  sliderImg.src =
    "https://anatomi.s3.eu-north-1.amazonaws.com/sompo_kasko_kare_image.jpg";
  ...
  slideTitle.textContent =
    "Kasko teklif hesaplama formunu doldurarak online olarak aracınız için en uygun kasko teklifini alabilirsiniz.";
  ...
  if (window.innerWidth < 768) {
    ...
    secondSlideMobilTitle.textContent = "Full Kasko Sigortası";
    secondSlide.appendChild(secondSlideMobilTitle);
  }

 
const items = [
  "<strong>%60’a varan</strong>  hasarsızlık indirimi",
  "Anlaşmalı kredi kartlarına <strong> 8 taksit imkanı</strong>",
  "Öğretmen, emekli, doktor, kamu personeli gibi 26 farklı meslek grubuna özel  <strong>30%’a varan indirim</strong>",
  "<strong>Ücretsiz</strong> mini onarım hizmeti",
  "Türkiye çapında 1300’e yakın anlaşmalı servislerimizde <strong>hiçbir ücret ödemeden tamir imkanı</strong> ",
  "EHOM Servislerinde <strong>Hasarsızlık Koruma Teminatı ve Garantili Onarım Hizmeti</strong> ",
];
...
const sompo_urun_detay_kasko_init = () => {
  sompo_urun_detay_kasko_html();
  sompo_urun_detay_kasko_css();
  sompo_urun_detay_kasko_js();
};
const sompo_urun_detay_kasko_condition = () => {
  const location =
...
if (sompo_urun_detay_kasko_condition()) {
  sompo_urun_detay_kasko_init();
}
