...
const icon_container = document.querySelector("#professional-whatsapp-passive");
icon_container.classList.remove("ease");
icon_container.classList.add("icon_container");
icon_container.innerText = "";
icon_container.removeAttribute("id");

// Anasayfadaki görünen wp icon grubun en üst parent divi
const a_wp_wrap = document.querySelector('#whatsapp-wrap');
a_wp_wrap.classList.add('a_wp_wrap')

// wp iconun tklandığında acılan side bar 
const active_pr_wp = document.querySelector('#professional-whatsapp-active')



//sepet iconunu konumlandıracagım div etiketinin classını seçiyorum
const anatomi_container = document.querySelector('.contentWrapper.col.col-12')
...
a_wp_wrap.addEventListener('click', function() {
    const active_wp = document.querySelector('.a_wp_wrap.active');
...
anatomi_container.insertAdjacentHTML('afterbegin',sepet_html)

// sepetim ikonu ve yazısının üstündeki sayacın htmlsini ve aynı zamanda foksiyonlarını kopyaladım ko
const a_count = document.querySelector('.fr.btn-black.btn-round.passive.text-center.cart-soft-count');
a_count.classList.add('anatomi_sayac');
const a_countHTML = a_count.outerHTML;
const yeniSayac = document.querySelector('.yeni_sayac');
yeniSayac.innerHTML = a_countHTML;

// wp_html kodlarının basıldığı yer.
