...
const anatomiBtnContainer = `
    <div class="a_container">
        ...
        <button class="a_btn" type="button" onclick="activeOverlay()">
            Beden Seç
        </button>
    </div>
`
...
const anatomiSizeContainer = `
    <div class="a_size_container" style="display: none">
        ${Array.from(sizes).map(item => {
            return "<div class='a_size' onclick='anatomiSelectSize(event)'>" + item.innerText + "</div>"
        }).join("")}
    </div>
`
...
const overlay = `<div class="overlay" onclick="closeOverlay()" style="display: none"></div>`
