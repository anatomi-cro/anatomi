...
var up_kargo_info = document.querySelectorAll('.ins-slider-page.ins-element-wrap.ins-element-sub-frame');
up_kargo_info.forEach(function (item) {
  item.style.display = 'none';
});
var up_kargo_info2 = document.querySelector('.ins-editable-dynamic-element-parent');
up_kargo_info2.style.display = 'none'; // kargo adı ve ücretinin bulunduğu divi seçip yine içindeki elementlere ulaşıyorum.

var hepsiJet = document.querySelector('.basket-fixed .uk-container .uk-grid').children[1].children[1];
hepsiJet.classList.add('anatomi_div');
var deger_sil = hepsiJet.children[1];
var degerText = hepsiJet.children[1].innerText; // kargo ücreti 0,00TL ise ücretsiz kargo yazısı ekleme

if (degerText == '0,00TL') {
  // kargonuz ücretsiz yazısını yukarıdaki kargo bilgisinden hepsijet yazısının karlısına taşıyorum.
  var anatomi_text = document.querySelectorAll('.editable.ins-element-editable');
  var yazi = anatomi_text[4].innerText = "Tebrikler, Kargonuz Ücretsiz!";
...
var uk_anatomi = document.querySelector('.uk-alert-success.uk-alert');

if (uk_anatomi !== null) {
  var uk_container = document.querySelectorAll('.uk-container');
  uk_container[11].insertAdjacentElement('afterbegin', siparis_ozeti);
  var success_bottom = document.querySelector('.ins-content-wrapper.ins-content-wrapper-489');
