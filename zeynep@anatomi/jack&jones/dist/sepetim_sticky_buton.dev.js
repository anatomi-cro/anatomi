...
var anatomi_span = document.querySelectorAll('.uk-container span');
var hepsiJet = anatomi_span[32].parentElement; //hepsijectin karsısında tutulan 0 tl kargo ücreti alıyorum ve bu koşulun hemen içinde en altta siliyorum

var deger_sil = anatomi_span[23].parentElement.children[1];
var degerText = anatomi_span[23].parentElement.children[1].innerText;
hepsiJet.classList.add('anatomi_div'); // bu koşulda eğer yukarıda tuttugumuz degerText 0,00 tl ye eşitse yani ücretsiz kargoysa kargonuz ücretsiz textini yazdırıyorum.
...
if (degerText == '0,00TL') {
  ...
  deger_sil.remove();
  var top_kargo_info = document.querySelector('#ins-purchase-trigger');
  ...
  var ücretli_kargo = document.createElement('div');
  ücretli_kargo.classList.add('anatomi_ücretli_kargo');
  ücretli_kargo.insertAdjacentHTML('afterbegin', anatomi_html); // burada da yarattığım divi hepsijet parent elementinin içine atıyorum.

  var _anatomi_span = document.querySelectorAll('.uk-container span');

  var _hepsiJet = _anatomi_span[23].parentElement;

  _hepsiJet.insertAdjacentElement('afterend', ücretli_kargo);

  var _top_kargo_info = document.querySelector('#ins-purchase-trigger');

  _top_kargo_info.remove();
}
