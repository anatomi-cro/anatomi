...
const tab_btn = document.querySelector('#tabBtn')

const basliklar = document.querySelectorAll("#tabBtn li");
const tabPanel = document.querySelector("#tabPanel");
const liCocuklar = Array.from(tabPanel.children).filter((child) => child.tagName === "LI");


// Başlıklara tıklama olayı eklemek için forEach döngüsü kullanılır
basliklar.forEach((baslik, index) => {
    baslik.addEventListener("click", () => {
        // Başlığın altındaki açıklamayı (li elementi) ilgili başlığın hemen altına ekleyin
        baslik.insertAdjacentElement('afterend', liCocuklar[index]);
...
head.insertAdjacentHTML("beforeend", a_style);
  ...
  // // Diğer başlıkların altındaki açıklamaları gizlemek için display özelliğini kullanabilirsiniz
        // basliklar.forEach((b, i) => {
        //     if (i !== index) {
        //         liCocuklar[i].style.display = "none";
        //     }
        // });
    ...
    //     // Tıklanan başlığın altındaki bilgiyi göster veya gizle
    //     if (liCocuklar[index].style.display === "block") {
    //       liCocuklar[index].style.display = "none";
    //     } else {
    //       liCocuklar[index].style.display = "block";
    //     }
    //   });
    // });
...
const style = `
<style>
main > .ins-preview-wrapper {
    display: none !important;
}
.a_cargo, .a_cargo_free {
    display: flex;
...
.a_cargo p, .a_cargo_free p {
    margin: 0 !important;
}

.a_cargo svg, .a_cargo_free svg {
    margin-right: 5px;
}
...
//içeride ürün var mı?
const isProduct = document.querySelectorAll(".product-list").length > 0;

//Eğer ki ürün var ise class ekleniyor
if (isProduct) {
  document
    .querySelectorAll(
      "section.section-basket .basket-fixed .uk-container div .uk-flex span"
    )
    .forEach((item) => {
      if (item.innerText === "HepsiJet") {
        item.parentElement.classList.add("a_cargo_container");
        item.nextElementSibling.classList.add("a_cargo_price");
      }
    });
  ...
  document
    ...
    .forEach((item) => {
      if (item.innerText == "Ara Toplam") {
        console.log(item.nextElementSibling.classList.add("a_price_container"));
      }
    });
}

const cargoContainer = document.querySelector(".a_cargo_container");

let price = document
  .querySelector(".a_price_container")
...
const cargoPrice = document.querySelector(".a_cargo_price");

if (isProduct && parseFloat(price) < 1500) {
  let price = document
    .querySelector(".a_price_container")
    .innerText.replace("TL", "");
  price = price.replace(",", ".");

  let cargoPriceValue = (1500 - price).toFixed(2);

  svg = `<svg style="margin-bottom: 3px" width="17" height="20" viewBox="0 0 17 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M11.7453 2.100019C11.7956 2.78136 11.8488 2.79552 11.9049 2.79552C12.0024 2.79552 12.0969 2.100019 12.1531 2.66521L13.6362 0.43572C13.7248 0.302574 13.6835 0.129767 13.5476 0.0447802C13.4087 -0.0402066 13.2285 -0.00054609 13.1399 0.1326L11.6567 2.36209C11.5681 2.4924 11.6094 2.66521 11.7453 2.100019Z" fill="#2948E8"/>
  ...
  cargoContainer.style.gridTemplateColumns = "1fr 1fr";
  cargoContainer.style.gridTemplateRows = "1fr 1fr";

  cargoPrice.style.justifySelf = "end";
  text = `<div class="a_cargo" style="grid-column: 1 / 3"> ${svg} ${cargoPriceValue} <p>TL değerinde ürün ekle, <b>Kargon Ücretsiz</b> Olsun!</p> </div>`;
  cargoContainer.insertAdjacentHTML("beforeend", text);
}

if (isProduct && parseFloat(price) > 1500) {
  svg = `<svg style="margin-bottom: 3px" width="18" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  `;
  text = `<div class="a_cargo_free"> <div>${svg}</div> <p>Tebrikler, Kargonuz Ücretsiz!</p> </div>`;
  cargoPrice.innerHTML = text;
}

document.querySelector("head").insertAdjacentHTML("beforeend", style);

const basketSummary = document.querySelector(
  ".basket-summary .basket-summary-detail"
);
basketSummary.children[0].remove();
basketSummary.children[1].remove();
basketSummary.children[2].remove();
